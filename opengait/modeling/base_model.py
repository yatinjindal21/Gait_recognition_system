...
from opengait.utils import Odict, mkdir
...
class BaseModel(MetaModel, nn.Module):
    ...
    def _load_ckpt(self, save_name):
        ...
        checkpoint = torch.load(save_name, map_location="cuda")
    ...
    def inference(self, rank):
        ...
        for inputs in self.test_loader:
            ...
            ipts_cuda = list(ipts)
            ipts_cuda[0] = [x.cuda() for x in ipts_cuda[0]]
            
            with torch.cuda.amp.autocast(enabled=self.engine_cfg['enable_float16']):
                retval = self.forward(tuple(ipts_cuda))
    ...
    def run_train(model):
        ...
        for inputs in model.train_loader:
            ...
            with torch.cuda.amp.autocast(enabled=model.engine_cfg['enable_float16']):
                retval = model(ipts)
    ...
    def run_test(model):
        ...
        eval_func = evaluator_cfg.get("eval_func", "identification")
