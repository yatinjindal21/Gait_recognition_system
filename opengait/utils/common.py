...
def ts2var(x, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"), **kwargs):
    return autograd.Variable(x, **kwargs).to(device)


def np2var(x, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"), **kwargs):
    return ts2var(torch.from_numpy(x), device=device, **kwargs)


def list2var(x, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"), **kwargs):
    return np2var(np.array(x), device=device, **kwargs)
